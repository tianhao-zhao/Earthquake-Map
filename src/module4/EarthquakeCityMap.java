...
public class EarthquakeCityMap extends PApplet {
	...
	private void addKey() {	
		...
		// legend for city land occean
		fill(color(255, 0, 0));
		triangle((float)(50+2.5), (float)(100+0), (float)(50+0), (float)(100+5), (float)(50+5), (float)(100+5));
	...
	private void printQuakes() 
	{
		...
		for (Marker cm : countryMarkers) {
			...
			if (!pq.containsKey(countryName)) {
				pq.put(countryName, 0);
			}
		}
		pq.put("OCCEAN QUAKES", 0);
		for (Marker m: quakeMarkers) {
			EarthquakeMarker em = (EarthquakeMarker) m;
			if (em.isOnLand()) {
				if (em instanceof LandQuakeMarker) {
					String countryName = ((LandQuakeMarker)em).getCountry();
					pq.put(countryName, pq.get(countryName)+1);
				}
			}
			else {
				String oq = "OCCEAN QUAKES";
				pq.put(oq, pq.get(oq)+1);
			}
		}
		for (String key : pq.keySet()) {
			if (pq.get(key)!=0) {
				System.out.println(key+" "+pq.get(key));
			}
		}
